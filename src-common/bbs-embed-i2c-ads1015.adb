...
package body BBS.embed.I2C.ADS1015 is
   ...
   --
   --  Check to see if configured device is present.  Assume that if
   --  reading the configuration register succeeds, the device is present.
   --
   function present(self : ADS1015_record) return Boolean is
      err  : err_code;
      conf : uint16;
   begin
      conf := self.hw.readm1(self.address, reg_config, err);
      if err = NONE then
         return True;
      else
         return False;
