...
Buffers are needed for using the I2C and SPI busses.  The general definitions are below:
\begin{lstlisting}
...
\indextype{buffer}
\indextype{buff\_ptr}
In most cases, these are not needed in the user code and are only used in the I2C, SPI, and related device drivers.  In most cases, only a few bytes are needed in the buffer and if memory is tight, \datatype{buff\_index} can be reduced in range.  Should a larger buffer be needed for some device, this range can be increased.
...
\begin{lstlisting}
function get_on(self : PS9685_record; chan : channel;
                error : out err_code) return uint16;
\end{lstlisting}
\indexfunc{get\_on}
Gets the on time for a channel
\begin{itemize}
  \item $self$ - The PCA9685 device to query.
  \item $chan$ - The channel number.
  \item $error$ - The error code from any I2C transactions.
  \item Returns the on time for the specified channel.
\end{itemize}

\begin{lstlisting}
function get_off(self : PS9685_record; chan : channel;
                 error : out err_code) return uint16;
\end{lstlisting}
\indexfunc{get\_of}
Gets the on time for a channel
\begin{itemize}
  ...
  \item $chan$ - The channel number.
  \item $error$ - The error code from any I2C transactions.
  \item Returns the off time for the specified channel.
\end{itemize}
...
\end{lstlisting}
\indextype{MWCR0\_MODE}
The datatype \datatype{MWCR0\_MODE} is used to determine if the RA8875 is operating in text mode or graphics mode.

\begin{lstlisting}
type MWCR0_CURDIR is (LRTD, RLTD, TDLR, DTLR);
\end{lstlisting}
\indextype{MWCR0\_CURDIR}
The datatype \datatype{MWCR0\_CURDIR} is used to determine the memory write direction for graphics mode.  The options are left to right top down (\constant{LRTD}), right to left top down (\constant{RLTD}), top down left to right (\constant{TDLR}), and down to top left to right (\constant {DTLR}).

\begin{lstlisting}
type MWCR1_GCURS_ENABLE is (disable, enable);
\end{lstlisting}
\indextype{MWCR1\_GCURS\_ENABLE}
The datatype \datatype{MWCR1\_GCURS\_ENABLE} is used to determine if the graphics cursor should be enabled or disabled.

\begin{lstlisting}
type MWCR1_GCURS_SET is range 0 .. 7;
\end{lstlisting}
\indextype{MWCR1\_GCURS\_SET}
The datatype \datatype{MWCR1\_GCURS\_SET} is used to select which set of graphics cursors to use.

\begin{lstlisting}
type MWCR1_WRITE_DEST is (LAYER, CGRAM, GCURS, PATTERN);
\end{lstlisting}
\indextype{MWCR1\_WRITE\_DEST}
The datatype \datatype{MWCR1\_WRITE\_DEST} is used to select the destination for graphics writing.
...
\end{lstlisting}
\indextype{FNCR0\_Code\_Page}
The datatype \datatype{FNCR0\_Code\_Page} is used to select which code page to use when translating character codes to glyphs on the display.

\begin{lstlisting}
type ELLIPSE_PART is (ELLIPSE_LL, ELLIPSE_UL, ELLIPSE_UR, ELLIPSE_LR);
\end{lstlisting}
\indextype{ELLIPSE\_PART}
The datatype \datatype{ELLIPSE\_PART} is used to select which part of an ellipse is drawn.  The options are lower-left (\constant{ELLIPSE\_LL}), upper-left (\constant{ELLIPSE\_UL}), upper-right (\constant{ELLIPSE\_UR}), and lower-right (\constant{ELLIPSE\_LR}).

\begin{lstlisting}
type LTPR0_SCROLL_MODE is (LAYER12_SIMULTANEOUS, LAYER1_ONLY,
                           LAYER2_ONLY, BUFFERED);
\end{lstlisting}
\indextype{LTPR0\_SCROLL\_MODE}
The datatype \datatype{LTPR0\_SCROLL\_MODE} is used to identify the mode for scrolling.  Both layers can be scrolled simultaneously, either of the two layers can be selected to scroll independently, or layer 2 can be used as a scroll buffer.

\begin{lstlisting}
type LTPR0_DISP_MODE is (ONLY_LAYER1, ONLY_LAYER2, LIGHTEN, TRANSPARENT,
                         BOOL_OR, BOOL_AND, FLOATING, RESERVED);
\end{lstlisting}
\indextype{LTPR0\_DISP\_MODE}
The datatype \datatype{LTPR0\_DISP\_MODE} is used to identify the way to display the two layers.  Each layer can be displayed by itself or in various combinations.
...
\begin{lstlisting}
procedure setWriteCtrl0(self : RA8875_record; mode : MWCR0_MODE;
                        cursorVisible : boolean; cursorBlink : boolean;
                        writeDir : MWCR0_CURDIR;
...
procedure setWriteCtrl1(self : RA8875_record;
                        cursorEnable : MWCR1_GCURS_ENABLE;
                        GCursorSelect : MWCR1_GCURS_SET;
                        writeDest : MWCR1_WRITE_DEST;
                        layer : RA8875_LAYER);
\end{lstlisting}
...
\begin{lstlisting}
procedure drawEllipseSegment(self : RA8875_record; x : uint16; y : uint16;
                             hRad : uint16; vRad : uint16; seg : ELLIPSE_PART;
                             color : R5G6B5_color; fill : boolean);
\end{lstlisting}
\indexfunc{drawEllipseSegment}
Draw a segment of an ellipse oriented either horizontally or vertically.
...
\begin{itemize}
  \item $self$ - The RA8875 device to check.
  \item Returns \constant{True} if a touch event has occurred.
\end{itemize}
