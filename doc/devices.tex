...
\color{red}{Warning!  The sysfs interface to GPIOs has been deprecated in Linux.  This package will have to be redone for the new \function{ioctl} based interface.  The current sysfs based package is renamed to \package{BBS.embed.GPIO.Linux\_sysfs} so that it can be used with legacy systems.}

\color{black}{As the general-purpose I/O pins are handled using device files (sometimes multiple files), the name of the device file has to be passed to a configure routine which then opens the file.  When finished, the file should be closed.}
